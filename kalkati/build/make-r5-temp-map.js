...
function $JSCompiler_StaticMethods_exportPack$$($JSCompiler_StaticMethods_exportPack$self$$, $write$$2$$, $typeList$$, $typeTbl$$, $nameList$$, $nameTbl$$, $newFormat$$) {
  var $codec$$2$$ = new $reach$data$Codec$$, $lat1_nodeNum$$6$$, $nodeCount$$6$$, $wayNum$$8$$, $wayCount$$8$$, $way$$9$$, $lon1_node$$16$$, $lat$$7_nameId$$, $lon$$7_typeId$$, $bit_latN$$, $lonN$$, $bit1$$, $bitN$$;
  ...
  for($wayNum$$8$$ = 0;$wayNum$$8$$ < $wayCount$$8$$;$wayNum$$8$$++) {
    ...
    $lat$$7_nameId$$ = $JSCompiler_StaticMethods_exportPack$self$$.$sEdge$;
    $lon$$7_typeId$$ = $JSCompiler_StaticMethods_exportPack$self$$.$wEdge$;
    $lat1_nodeNum$$6$$ = $way$$9$$.$nodeList$[0].$ll$.$llat$;
    $lon1_node$$16$$ = $way$$9$$.$nodeList$[0].$ll$.$llon$;
    $bit_latN$$ = $way$$9$$.$nodeList$[$nodeCount$$6$$ - 1].$ll$.$llat$;
    $lonN$$ = $way$$9$$.$nodeList$[$nodeCount$$6$$ - 1].$ll$.$llon$;
    $bitN$$ = $bit1$$ = 0;
    if($lat1_nodeNum$$6$$ < $JSCompiler_StaticMethods_exportPack$self$$.$sEdge$ || $lat1_nodeNum$$6$$ >= $JSCompiler_StaticMethods_exportPack$self$$.$nEdge$ || $lon1_node$$16$$ < $JSCompiler_StaticMethods_exportPack$self$$.$wEdge$ || $lon1_node$$16$$ >= $JSCompiler_StaticMethods_exportPack$self$$.$eEdge$) {
      $bit1$$ = 1
    }
    if($bit_latN$$ < $JSCompiler_StaticMethods_exportPack$self$$.$sEdge$ || $bit_latN$$ >= $JSCompiler_StaticMethods_exportPack$self$$.$nEdge$ || $lonN$$ < $JSCompiler_StaticMethods_exportPack$self$$.$wEdge$ || $lonN$$ >= $JSCompiler_StaticMethods_exportPack$self$$.$eEdge$) {
      $bitN$$ = 1
    }
    for($lat1_nodeNum$$6$$ = 0;$lat1_nodeNum$$6$$ < $nodeCount$$6$$;$lat1_nodeNum$$6$$++) {
      $lon1_node$$16$$ = $way$$9$$.$nodeList$[$lat1_nodeNum$$6$$];
      $lon1_node$$16$$.$clusterRef$ && ($lon1_node$$16$$ = $lon1_node$$16$$.$clusterRef$);
      if($newFormat$$) {
        $bit_latN$$ = 0;
        1 < $lon1_node$$16$$.$wayList$.length && ($bit_latN$$ = 1);
        ...
        $write$$2$$($JSCompiler_StaticMethods_encodeLong$$($codec$$2$$, [2 * $reach$util$fromSigned$$($lon1_node$$16$$.$ll$.$llat$ - $lat$$7_nameId$$) + $bit_latN$$, $reach$util$fromSigned$$($lon1_node$$16$$.$ll$.$llon$ - $lon$$7_typeId$$)]))
      }else {
        $write$$2$$($JSCompiler_StaticMethods_encodeLong$$($codec$$2$$, [$reach$util$fromSigned$$($lon1_node$$16$$.$ll$.$llat$ - $lat$$7_nameId$$), $reach$util$fromSigned$$($lon1_node$$16$$.$ll$.$llon$ - $lon$$7_typeId$$)]))
      }
      $lat$$7_nameId$$ = $lon1_node$$16$$.$ll$.$llat$;
      $lon$$7_typeId$$ = $lon1_node$$16$$.$ll$.$llon$
    }
  }
}
...
Fiber(function compute() {
  ...
  $data$$51_tree$$.forEach(function($tile$$17$$) {
    ...
    }, $typeList$$4$$, {}, $nameList$$6$$, {}, $JSCompiler_alias_TRUE$$), $fd$$2$$ = fs.openSync("../tiles-r5/" + $tile$$17$$.path + ".txt", "w"), $writeStringList$$($typeList$$4$$), $writeStringList$$($nameList$$6$$), $write$$9$$($data$$54$$.join("")), fs.closeSync($fd$$2$$))
  });
